...
mongoose.connect('mongodb://alfredo:hjk68101055@ac-2nmnw00-shard-00-00.dbaa8ha.mongodb.net:27017,ac-2nmnw00-shard-00-01.dbaa8ha.mongodb.net:27017,ac-2nmnw00-shard-00-02.dbaa8ha.mongodb.net:27017/Citas?ssl=true&replicaSet=atlas-viltrt-shard-0&authSource=admin&retryWrites=true&w=majority&appName=Cluster0')
...
app.get('/citas', async (req, res) =>{
    await citaModel.find()
    .then(data => res.json(data))
    .catch(err => res.json(err))
})
 app.get('/tratamientos', async (req, res) =>{
    await tratamientoModel.find()
    .then(data => res.json(data))
    .catch(err => res.json(err))
 })
...
app.get('/citas/:_id', async (req, res) =>{
    ...
    await citaModel.find({_id:citasId})
    .then(data => res.json(data))
    .catch(err => res.json(err))
})
...
app.get('/tratamientos/:_id', async (req, res) =>{
    ...
    await tratamientoModel.find({_id:tratamientoId})
    .then(data => res.json(data))
    .catch(err => res.json(err))
})
...
app.post('/citas', async (req, res) =>{
    const citas = new citaModel(req.body);
...
app.post('/tratamientos', async (req, res) =>{
    const tratamientos = new tratamientoModel(req.body);
...
app.delete('/citas/:_id', async (req, res) =>{
    const citasId = req.params._id;
    const result = await citaModel.deleteOne({_id:citasId});
    res.json(result)
})

app.delete('/tratamientos/:_id', async (req, res) =>{
    const tratamientoId = req.params._id;
    const result = await tratamientoModel.deleteOne({_id:tratamientoId})
    res.json(result)
})
...
app.put('/citas/:_id', async (req, res) =>{
    const citasId = req.params._id;
    const result = await citaModel.findByIdAndUpdate({_id:citasId}, {
...
app.put('/tratamientos/:_id', async (req, res) =>{
    const tratamientoId = req.params._id;
    const result = await tratamientoModel.findByIdAndUpdate({_id:tratamientoId}, {
